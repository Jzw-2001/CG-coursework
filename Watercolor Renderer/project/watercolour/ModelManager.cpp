...
bool ModelManager::loadModel(const std::string& filePath, const glm::vec3& pos, const glm::vec3& initialRotation) {
    ...
    for (auto& mesh : Loader.LoadedMeshes) {
		...
		boatMeshEntries.push_back(entry);
    }
    return true;
}
...
void ModelManager::drawBoatModel(GLuint shaderProgram, glm::vec3 lightPos, glm::vec3 cameraPosition, GLuint shadowMap, glm::mat4 lightSpaceMatrix) {
	...
	for (auto& entry : boatMeshEntries) {
		glBindVertexArray(entry.VAO);
